...
describe('Download Utilities', () => {
  describe('fileExists', () => {
    it('should check if file exists', async () => {
      // Since this requires actual file system, just test the function exists
  ...
  describe('delay', () => {
    it('should delay for specified time', async () => {
      ...
      // Check that it took at least close to 10ms (allowing for some variation)
      assert.ok(elapsed >= 8, `Expected delay of ~10ms, got ${elapsed}ms`)
    })
  })
})
...
describe('Artist Title Parsing', () => {
  it('should parse non-Latin characters properly', async () => {
    // Import get-artist-title to test its functionality
    const getArtistTitle = (await import('get-artist-title')).default;
    
    // Test various non-Latin character patterns
    const cyrillicResult = getArtistTitle('Песня Исполнитель - Заголовок трека');
    assert.deepStrictEqual(cyrillicResult, ['Песня Исполнитель', 'Заголовок трека']);
    
    const chineseResult = getArtistTitle('测试 音乐家 - 测试歌曲');
    assert.deepStrictEqual(chineseResult, ['测试 音乐家', '测试歌曲']);
    
    const accentedResult = getArtistTitle('Café Résistance - Nouveau Monde');
    assert.deepStrictEqual(accentedResult, ['Café Résistance', 'Nouveau Monde']);
    
    const mixedResult = getArtistTitle('Artist Тест - Song Title');
    assert.deepStrictEqual(mixedResult, ['Artist Тест', 'Song Title']);
    
    const cyrillicSimple = getArtistTitle('Музыка - Песня');
    assert.deepStrictEqual(cyrillicSimple, ['Музыка', 'Песня']);
    
    // Test cases that should return undefined (no pattern match)
    const noMatch = getArtistTitle('Just a Song Title');
